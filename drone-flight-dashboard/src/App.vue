    ...
    <div v-else-if="isAuthenticated" class="app-layout">
      ...
      <div class="main-container">
        ...
        <main class="content">
          ...
          <TopoMap v-else-if="currentView === 'topography'" />
...
.content {
  flex: 1;
  margin-top: 60px; 
  /* Учитываем фиксированный сайдбар, чтобы контент не прятался слева */
  margin-left: 280px;
  overflow-y: auto;
  background: #f9fafb;
  padding: 24px;
  /* Блоковый поток вместо flex, чтобы контент не прилипал к левому краю */
  display: block;
  width: calc(100% - 280px);
